...
for sample in samples:
  ...
  #bounds = [15,25,20,80,90,100]

  #if paretoRanks[sample.id]<50: nPar+=1
  #if (exx<bounds[0] and eyy<bounds[1] and ezz<bounds[2] and pf1<bounds[3] and pf2<bounds[4] and pf2<bounds[5]): nErr+=1
  #if (exx<bounds[0] and eyy<bounds[1] and ezz<bounds[2] and pf1<bounds[3] and pf2<bounds[4] and pf2<bounds[5]) and paretoRanks[sample.id]<50: nBof+=1
  #if (exx<bounds[0] and eyy<bounds[1] and ezz<bounds[2] and pf1<bounds[3] and pf2<bounds[4] and pf2<bounds[5]) and paretoRanks[sample.id]<50:

  if (exx<bounds[0] and eyy<bounds[1] and ezz<bounds[2] and pf1<bounds[3] and pf2<bounds[4] and pf2<bounds[5]):
    print 'alive, sample %i', sample.id
  else:
    opts[1].misc['dead_ids'] += [sample.id]
    opts[2].misc['dead_ids'] += [sample.id]

print nErr,nPar,nBof
