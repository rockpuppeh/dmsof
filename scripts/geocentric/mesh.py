...
for icurve in cubit.get_entities('curve'):
  ...
  if cubit.get_relatives('curve',icurve,'volume')>36:
    if np.abs(l-r1)<0.01 or np.abs(l-r2)<0.01 or np.abs(l-r3)<0.01:
  ...
  for icurve in cubit.get_relatives('volume',ivol,'curve'):
    ...
    if len(v)==2:
      ...
      if ivol in range(1,29) or ivol in range(33,37):
        if np.abs(y0-y1)<0.01 and np.abs(z0-z1)<0.01 and np.abs(0.5*(x0+x1)-0)>0.1:
          bias = 0.91
          intv = 5
          if 0.5*(x0+x1)<0:
        ...
        if np.abs(x0-x1)<0.01 and np.abs(z0-z1)<0.01 and np.abs(0.5*(y0+y1)-0)>0.1:
          bias = 0.91
          intv = 5
          if 0.5*(y0+y1)<0:
...
for ivol in range(1,cubit.get_volume_count()+1):
  for ihex in cubit.get_volume_hexes(ivol):
    ...
    elif -7<=z and z<=0:  hex_types = np.concatenate( [hex_types, np.array([2]).reshape(1,1)], axis=0 )
